...
// NewUsers map[string]UserConfig
func NewUsers(config map[string]UserConfig) error {
	for key, userConfig := range config {
		cert, prikey, err := FindCertAndKeyFile(userConfig.MspConfigPath)
		if err != nil {
			return errors.WithMessage(err, "find cert and key file failed")
		}
		users[key], err = loadCertFromFile(cert, prikey)
		if err != nil {
			return errors.WithMessage(err, "load cert failed")
		}
		users[key].MspId = userConfig.MspID
	}
	return nil
}
